    ...
    // TODO: implement load logic using fetch(...)
        try {
           ...
           const data=await response.json();
            ...
            setStatus("Error loading tasks: " + err, true);
        }
});
...
async function deleteTask(id, liElement) {
    ...
    try {
        const url = `${API_BASE}/delete.php?stdid=${encodeURIComponent(STUDENT_ID)}&key=${encodeURIComponent(API_KEY)}&id=${id}`;
